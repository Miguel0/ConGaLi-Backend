...
class ConwaysGame {
  ...
  createCellsBy (userId, cellsGridId, points, done) {
    this.cellsGrids[cellsGridId].addCellsBy(this.users[userId], points)
    done()
  }

  createCellsOfTemplateBy (templateCreationData, userId) {
    let templateGroups = this.getPresetsConfiguration()
    let template

    for (let group in templateGroups) {
    ...
    if (template) {
      this.createCellsBy(userId, 0, template.points)
      done(null, this)
    } else {
      done(new AppException(
